...
export default function ToolCard({ tool, onClick }: ToolCardProps) {
  ...
  const getCategoryInsight = () => {
    if (tool.tags?.includes('email')) return 'Email investigation and verification';
    if (tool.tags?.includes('domain')) return 'Domain analysis and reconnaissance';
    if (tool.tags?.includes('social-media')) return 'Social media intelligence gathering';
    if (tool.description.toLowerCase().includes('search')) return 'Search and discovery operations';
    if (tool.description.toLowerCase().includes('scan')) return 'Security scanning and analysis';
            ...
            <div className="flex items-center space-x-3">
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className={`w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center hover:bg-blue-200 transition-colors`}>
                ...
                <TooltipContent side="top" className="max-w-xs">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{getCategoryInsight()}</p>
                  </div>
                </TooltipContent>
              </Tooltip>
              <div>
                    ...
                    <TooltipContent side="top" className="max-w-sm">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2 pt-2 border-t border-border">
                          ...
                          <span className="text-xs text-muted-foreground">{getCategoryInsight()}</span>
                        </div>
                      </div>
                    </TooltipContent>
                  </Tooltip>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom">
                      <p className="text-xs">
                        {tool.pricing === 'Free' ? 'No cost to use this tool' : 
                         tool.pricing === 'Freemium' ? 'Basic features free, premium features require payment' :
                  ...
                  {tool.isOfficial && (
                    <Tooltip>
                      ...
                      <TooltipContent side="bottom">
                        <p className="text-xs">Verified and trusted tool from official source</p>
                      </TooltipContent>
                    </Tooltip>
                  )}
                  {tool.hasApi && (
                    <Tooltip>
                      ...
                      <TooltipContent side="bottom">
                        <p className="text-xs">Programmatic access available via API</p>
                      </TooltipContent>
                    </Tooltip>
                  )}
                  {tool.platform === 'GitHub' && (
                    <Tooltip>
                      ...
                      <TooltipContent side="bottom">
                        <p className="text-xs">Open source tool available on GitHub</p>
                      </TooltipContent>
                    </Tooltip>
                  )}
                </div>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom" className="max-w-md">
              <div className="space-y-2">
                ...
                <p className="text-sm">{tool.fullDescription || tool.description}</p>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 text-xs text-slate-500">
              ...
              <Tooltip>
                ...
                <TooltipContent side="bottom">
                  <p className="text-xs">Estimated number of active investigators using this tool</p>
                </TooltipContent>
              </Tooltip>
            </div>
            <div className="flex items-center space-x-2">
